...
psql -v ON_ERROR_STOP=1 "postgresql://${DB_USER}:${DB_PASS}@${DB_HOST}/${DB_NAME}" -f /usr/src/app/db_refresh.sql  || exit 1
echo "Db refresh Complete!"
psql -v ON_ERROR_STOP=1 "postgresql://${DB_USER}:${DB_PASS}@${DB_HOST}/${DB_NAME}" -f /usr/src/app/db_queries.sql ||  exit 1
