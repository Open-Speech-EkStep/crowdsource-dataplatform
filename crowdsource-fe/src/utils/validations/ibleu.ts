...
function generalComputeBLEU(
  ...
  let sum = 0.0;
  for (i = 1; i <= order; i++) {
    sum = scores.slice(1, i + 1).reduce(function (x: any, y: any) {
      return x + y;
    });
  ...
  let cumulative = prec.map(function (x: number) {
    return x * brevity;
  });
...
function bleuScoreObject(order: number, smoothed: boolean) {
  //4, true
  ...
  obj.computeBLEU = function () {
    ...
    let bleuResults = generalComputeBLEU(
      obj.reflength,
      obj.ngramMatches,
      obj.numTstNgrams,
      obj.smoothed,
...
export function scoreSegment(tstSegment: string, refSegment: string) {
  ...
  for (let ngram in refNgrams) {
    clippedRefNgrams[ngram] = isUndefined(clippedRefNgrams[ngram])
      ? refNgrams[ngram]
      : Math.max(clippedRefNgrams[ngram], refNgrams[ngram]);
  }
  ...
  // }
    ...
    let diffs = reflengths.map(function (x: number) {
      return Math.abs(x - tstlength);
    });
