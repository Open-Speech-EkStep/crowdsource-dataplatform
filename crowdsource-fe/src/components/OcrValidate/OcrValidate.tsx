...
import styles from './OcrValidate.module.scss';
...
const OcrValidate = () => {
  ...
  const [formData, setFormData] = useState<ActionStoreInterface>({
    ...
    type: INITIATIVES_MEDIA_MAPPING.ocr,
    sentenceId: 0,
    state: '',
    country: '',
  ...
  useEffect(() => {
    if ((storeData || skipData || acceptData) && !(skipError || submitError || acceptError)) {
      if (currentDataIndex === contributionData.length) {
        router.push(`/${currentLocale}${routePaths.ocrInitiativeValidateThankYou}`, undefined, {
    ...
    <Fragment>
      {contributionData && result?.data?.length !== 0 ? (
        <Fragment>
          ...
          <div className="pt-4 px-2 px-lg-0 pb-8">
            <FunctionalHeader
              ...
              action={INITIATIVE_ACTIONS[INITIATIVES_MAPPING.ocr]['validate']}
              showSpeaker={false}
            />
            <Container fluid="lg" className="mt-5">
              <div data-testid="OcrValidate" className={`${styles.root}`}>
                <div className="align-items-center text-center">
                  <span className="display-3">{t(`${INITIATIVES_MAPPING.ocr}ValidationHeading`)}</span>
                </div>
                <div className="mt-2 mt-md-4">
                  {showUIData?.sentence && (
                    <ImageView
                      imageUrl={encodeURIComponent(showUIData?.sentence)}
                      expandState={viewExpand}
                      handleExpandView={expandState => setViewExpand(expandState)}
                    />
                  )}
                </div>
                {showEditTextBlock ? (
        ...
        <div className="d-flex flex-grow-1 align-items-center">
          <NoDataFound
            url={routePaths.ocrInitiativeHome}
            title={t('ocrValidateNoDataThankYouMessage')}
