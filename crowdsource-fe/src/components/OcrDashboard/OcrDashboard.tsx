...
const OcrDashboard = () => {
  ...
  const contributePageUrl = `/${router.locale}${routePaths[`ocrInitiativeContribute`]}`;
  ...
  return (
    <div className="pt-4 px-2 px-lg-0 pb-8">
      <header className="d-flex justify-content-between align-items-center px-3 px-md-6">
        <Breadcrumbs initiative={INITIATIVES_MAPPING.ocr} path="dashboard" />
      </header>
      <Container fluid="lg" className="mt-5">
        ...
        <Fragment>
          <div className="mt-8">
            {(!language && <ContributionStats showComponent={true} initiative={INITIATIVES_MAPPING.ocr} />) ||
              (language && (
          ...
          <Row className="mt-10">
            <Col lg="6" data-testid="GeographicalChart">
              <IndiaMapChart type={INITIATIVES_MEDIA_MAPPING.ocr} language={language} />
            </Col>
            <Col lg="6" className="mt-8 mt-lg-0" data-testid="ProgressChart">
              <ProgressChart type={INITIATIVES_MEDIA_MAPPING.ocr} language={language} />
            </Col>
          </Row>
        </Fragment>
      </Container>
