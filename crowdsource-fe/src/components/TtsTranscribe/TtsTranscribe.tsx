...
import styles from './TtsTranscribe.module.scss';
...
const TtsTranscribe = () => {
  ...
  const [formData, setFormData] = useState<ActionStoreInterface>({
    ...
    type: INITIATIVES_MEDIA_MAPPING.tts,
    sentenceId: 0,
    state: '',
    country: '',
  ...
  useEffect(() => {
    if ((storeData || skipData) && !(skipError || submitError)) {
      if (currentDataIndex === contributionData.length) {
        router.push(`/${currentLocale}${routePaths.ttsInitiativeContributeThankYou}`, undefined, {
    ...
    <Fragment>
      {contributionData && result?.data?.length !== 0 ? (
        <Fragment>
          ...
          <div className="pt-4 px-2 px-lg-0 pb-8">
            <FunctionalHeader
              ...
              action={INITIATIVE_ACTIONS[INITIATIVES_MAPPING.tts]['contribute']}
            />
            <Container fluid="lg" className="mt-5">
              <div data-testid="TtsTranscribe" className={`${styles.root}`}>
                <div className="d-flex flex-column align-items-center text-center">
                  <span className="display-3">{t(`ttsContributionHeading`)}</span>
                  <div className="mt-2 mt-md-3">
                    ...
                    <audio
                      ...
                      src={
                        ...
                        `${nodeConfig.cdnUrl}/${encodeURIComponent(showUIData?.media_data)}`
                      }
                ...
                <div className="mt-4 mt-md-8">
                  <TextEditArea
                    ...
                    language={contributionLanguage ?? ''}
                    initiative={INITIATIVES_MEDIA_MAPPING.tts}
        ...
        <div className="d-flex flex-grow-1 align-items-center">
          <NoDataFound
            url={routePaths.ttsInitiativeHome}
            title={t('ttsContributeNoDataThankYouMessage')}
            text={t('noDataMessage', { language: t(`${contributionLanguage?.toLowerCase()}`) })}
            buttonLabel={t('ttsBackToInitiativePrompt')}
          />
        </div>
      )}
      {(error || skipError || submitError) && (
