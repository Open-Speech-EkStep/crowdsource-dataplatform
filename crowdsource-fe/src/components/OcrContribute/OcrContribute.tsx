...
import styles from './OcrContribute.module.scss';
  ...
  useEffect(() => {
    if ((storeData || skipData) && !(skipError || submitError)) {
      if (currentDataIndex === contributionData.length) {
        router.push(`/${currentLocale}${routePaths.ocrInitiativeContributeThankYou}`, undefined, {
        ...
        <Fragment>
          ...
          <div className="pt-4 px-2 px-lg-0 pb-8">
            <FunctionalHeader
              ...
              action={INITIATIVE_ACTIONS[INITIATIVES_MAPPING.ocr]['contribute']}
              showSpeaker={false}
            />
            <Container fluid="lg" className="mt-5">
              <div data-testid="OcrContribute" className={`${styles.root}`}>
                <div className="align-items-center text-center">
                  <span className="display-3">{t(`${INITIATIVES_MAPPING.ocr}ContributionHeading`)}</span>
                </div>
                <div className="mt-2 mt-md-4">
                  {showUIData?.media_data && (
                    <ImageView
                      imageUrl={encodeURIComponent(showUIData?.media_data)}
