...
const BadgeDetail = () => {
  ...
  const [defaultInitiative, setDefaultInitiative] = useState<Initiative>(
    INITIATIVES_REVERSE_MEDIA_MAPPING[initiative as InitiativeType] || INITIATIVES_MAPPING.tts
  );
  ...
  const onSetInitiativeRouting = (initiativeType: InitiativeType) => {
    setRoutingParam(initiativeType, defaultLanguage, defaultAction, defaultBadge);
  };
  ...
  function handleTabClick(tab: string | null, event: any) {
    ...
    onSetInitiativeRouting(INITIATIVES_MEDIA_MAPPING[tab as Initiative]);
  }

  useEffect(() => {
    const initiativeTarget = document.getElementById('uncontrolled-tab-example-tab-' + defaultInitiative);
    setTimeout(() => {
      initiativeTarget?.scrollIntoView({
        behavior: 'smooth',
        block: 'nearest',
        inline: 'center',
      });
        ...
        >
          {Object.values(INITIATIVES_MAPPING).map((initiativeName: string) => (
            <Tab
              ...
              title={`${t(initiativeName)} ${t('initiativeTextSuffix')}`}
