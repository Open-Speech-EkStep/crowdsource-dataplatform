...
describe('TranslationTranslate', () => {
  ...
  const setup = async (result: any) => {
    ...
    const renderResult = render(<TranslationTranslate />);
  ...
  it('should show the thank you message when no data present', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('translationContributeNoDataThankYouMessage')).toBeInTheDocument();
    });
  });
