...
import { convertTimeFormat, isTtsOrAsrInitiative } from 'utils/utils';
...
const getTopLanguagesByHoursChartData = (
  ...
  return (
    topLanguagesByHours?.map(topLanguageByHours => ({
      ...
      tooltipText: isTtsOrAsrInitiative(initiativeMedia)
        ? convertTimeFormat(topLanguageByHours?.total_contributions)
        : `${topLanguageByHours?.total_contribution_count} ${i18n
            ?.t(`${INITIATIVES_REVERSE_MEDIA_MAPPING[initiativeMedia]}BarGraphTooltip`)
            ?.toLocaleLowerCase()}`,
    })) ?? []
  );
};
...
const getTopLanguagesBySpeakerChartData = (
  ...
  return (
    topLanguagesBySpeaker?.map(topLanguagesBySpeaker => ({
      ...
      tooltipText:
        (topLanguagesBySpeaker?.total_speakers || '0') +
        ' ' +
        i18n?.t(`${INITIATIVES_REVERSE_MEDIA_MAPPING[type]}BarGraphTooltip`),
    })) ?? []
  );
};
    ...
    () => ({
      data:
        chartFilterType === 'byDuration'
          ...
          : props.initiative === INITIATIVES_MAPPING.tts
          ? getTopLanguagesBySentencesChartData(
              INITIATIVE_MEDIA_CONTRIBUTION_MAPPING[INITIATIVES_MEDIA_MAPPING.ocr],
              INITIATIVES_MEDIA_MAPPING[props.initiative],
              topLanguageHrsData
            )
      ...
      yAxisLabel:
        chartFilterType === 'byDuration'
          ? t(`${INITIATIVES_MAPPING[props.initiative]}ContributionGraphYLabel1`)
          : t(`${INITIATIVES_MAPPING[props.initiative]}ContributionGraphYLabel2`),
      bgColor: '#333',
      ...chartLegendDetails,
    }),
