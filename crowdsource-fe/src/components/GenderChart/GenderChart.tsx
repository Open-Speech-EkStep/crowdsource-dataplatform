...
const chartYLabel = 'asrContributionGraphYLabel1';
...
const GenderChart: FunctionComponent<GenderChartProps> = ({ language }) => {
  ...
  if (maleData)
    ...
    <div>
        ...
        <div>${t('asrBarGraphTooltip')}: <label>${maleData?.speakers}</label></div>
    </div>`,
    });
  ...
  if (femaleData)
    ...
    <div>
        ...
        <div>${t('asrBarGraphTooltip')}: <label>${femaleData?.speakers}</label></div>
    </div>`,
    });
  ...
  if (notSpecifiedData)
    ...
    <div>
        ...
        <div>${t('asrBarGraphTooltip')}: <label>${notSpecifiedData?.speakers}</label></div>
    </div>`,
    });
  ...
  othersDataList?.forEach(item => {
    ...
    chartData.push({
      ...
      <div>
          ...
          <div>${t('asrBarGraphTooltip')}: <label>${item?.speakers}</label></div>
      </div>`,
    });
  });
