...
describe('TargetProgress No Data', () => {
  const setup = async (
    initiative: 'tts' | 'asr' | 'ocr' | 'translation',
    initiativeMedia: 'asr' | 'ocr' | 'parallel' | 'text',
    source?: 'contribute' | 'validate',
  ...
  it('should render the result for ocr initiative for home page with both the data', async () => {
    await setup('ocr', 'ocr', undefined);
    expect(screen.getByText('ocrProgressStatus')).toBeInTheDocument();
    expect(
      screen.getByText(content => {
        return content.includes('9/1 images');
      })
    ).toBeInTheDocument();
  });

  it('should render the result for tts initiative home page with null contribution goal', async () => {
    await setup('tts', 'asr', 'contribute');
    expect(screen.getByText('ttsProgressStatus')).toBeInTheDocument();
    expect(
      screen.getByText(content => {
        return content.includes('1 minutes1/1 hours1');
      })
    ).toBeInTheDocument();
  });
});
