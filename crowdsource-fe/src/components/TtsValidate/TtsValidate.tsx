...
import styles from './TtsValidate.module.scss';
  ...
  useEffect(() => {
    if ((storeData || skipData || acceptData) && !(skipError || submitError || acceptError)) {
      if (currentDataIndex === contributionData.length) {
        router.push(`/${currentLocale}${routePaths.ttsInitiativeValidateThankYou}`, undefined, {
      ...
      {contributionData && result?.data?.length !== 0 ? (
        <Fragment>
          ...
          <div className="pt-4 px-2 px-lg-0 pb-8">
            <FunctionalHeader
              ...
              action={INITIATIVE_ACTIONS[INITIATIVES_MAPPING.tts]['validate']}
            />
            <Container fluid="lg" className="mt-5">
              <div data-testid="TtsValidate" className={`${styles.root}`}>
                <div className="d-flex flex-column align-items-center text-center">
                  <span className="display-3">{t(`ttsValidationHeading`)}</span>
        ...
        <div className="d-flex flex-grow-1 align-items-center">
          <NoDataFound
            ...
            title={t('ttsValidateNoDataThankYouMessage')}
