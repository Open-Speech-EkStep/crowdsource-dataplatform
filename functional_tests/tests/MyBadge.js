...
step("when user navigates to <tab> tab user should see badges", async function (tab) {
    ...
    if (tab == "ocr-tab") {
        await scrollUp(5000);
        await click(button('Dekho India'));
        await taiko.waitFor(1000);
        ...
        await button('Bronze', below('Validation')).exists();
        await button('Silver', below('Validation')).exists();
        await button('Gold', below('Validation')).exists();
        await button('Bronze', below('Contribution')).exists();
        await button('Silver', below('Contribution')).exists();
