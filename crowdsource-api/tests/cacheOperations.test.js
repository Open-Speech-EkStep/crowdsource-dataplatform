...
describe("Running tests for cacheOperations", () => {
    ...
    test("getDataForContribution should call set data when data is null", async () => {
        const type = 'asr'
        const userId = 'userId';
        const userName = 'userName';
        ...
        when(mockCache.getAsync).calledWith(`dataset_row_${type}_${language}_${toLanguage}`).mockReturnValueOnce(Promise.resolve(mockResult)).mockReturnValueOnce(Promise.resolve(JSON.stringify(mockResult1)));

        const result = await cacheOperations.getDataForContribution(type, language, toLanguage, userId, userName, mockDB);
