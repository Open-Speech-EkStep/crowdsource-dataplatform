...
AS
 SELECT contributions.contributed_by,
   ...
   FROM contributions
     LEFT JOIN validations ON contributions.contribution_id = validations.contribution_id AND COALESCE(validations.action, '') <> 'skip'
...
AS
    ...
    count(*) AS contributions,
    sum(contributions_and_demo_stats.contribution_audio_duration) FILTER (WHERE contributions_and_demo_stats.audio_row_num_per_contribution_id = 1 AND contributions_and_demo_stats.is_system = false)::numeric / 3600::numeric AS hours_contributed,
    sum(contributions_and_demo_stats.validation_audio_duration) FILTER (WHERE contributions_and_demo_stats.audio_row_num_per_contribution_id = 1 AND contributions_and_demo_stats.is_system = false)::numeric / 3600::numeric AS hours_validated,
    count(DISTINCT contributions_and_demo_stats.contributed_by) AS speakers
   FROM contributions_and_demo_stats_newest contributions_and_demo_stats
  WHERE contributions_and_demo_stats.type = 'text'::text
...
AS
   ...
   FROM ( SELECT contributions_and_demo_stats.contributions_state_region AS state,
     ...
     FULL JOIN ( SELECT contributions_and_demo_stats.validations_state_region AS state,
           ...
           FROM contributions_and_demo_stats_newest contributions_and_demo_stats
          WHERE contributions_and_demo_stats.validated_by IS NOT NULL
...
AS
  SELECT COALESCE(contribution_by_language.language, validation_by_language.language) AS language,
    ...
    round(sum(COALESCE(contribution_by_language.total_contributions, 0::bigint::double precision))::numeric / 3600::numeric, 3) AS total_contributions,
    round(sum(COALESCE(validation_by_language.total_validations, 0::bigint::double precision))::numeric / 3600::numeric, 3) AS total_validations,
    ...
    sum(COALESCE(validation_by_language.total_validation_count, 0::bigint::numeric)) AS total_validation_count
