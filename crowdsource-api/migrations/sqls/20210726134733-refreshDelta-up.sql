...
update contributions set datetime = date;

ALTER TABLE validations
    ADD COLUMN datetime timestamp with time zone NOT NULL DEFAULT CURRENT_TIMESTAMP;
    
update validations set datetime = date;

select contributed_by, contribution_id, validated_by, validation_id, date::timestamptz, gender, age_group, mother_tongue, contributions_state_region, validations_state_region, contribution_audio_duration, audio_row_num_per_contribution_id, language, "sentenceId", is_validated, validation_audio_duration, type, is_system into dashboard_stats from contributions_and_demo_stats;

-- update dashboard_stats ds set date = GREATEST(contributions.datetime, validations.datetime)
-- FROM contributions
...
--   WHERE ds.contribution_id=contributions.contribution_id and coalesce(ds.validation_id,0) = coalesce(validations.validation_id,0) and 
--   contributions.action = 'completed'::text 
--   AND contributions.datetime >= (now() - '1 day'::interval) AND (conf.value = 1 OR dataset_row.is_profane = false)
--   ;
