...
AS
 SELECT contributions.contributed_by,
  ...
  WHERE COALESCE(master_dataset.is_active, true) = true AND (contributions.is_system = false OR validations.action = ANY (ARRAY['accept'::text, 'reject'::text])) AND COALESCE(validations.action, '') <> 'skip' AND contributions.action = 'completed'::text AND contributions.date >= (CURRENT_DATE - '1 year'::interval) AND (conf.value = 1 OR dataset_row.is_profane = false)
