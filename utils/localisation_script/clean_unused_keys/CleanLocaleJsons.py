...
if __name__ == '__main__':
    ...
    group.add_argument("-l", "--languages", nargs="+", help = "Clean json for the languages mentioned by language codes(space separated)", choices=list(LANGUAGES.keys()))
    parser.add_argument("-i", "--input-folder-path", required=True, help = "Input folder path with json files present")
    parser.add_argument("-o", "--output-folder-path", required=True, help = "Output folder path to save cleaned jsons files")
