...
function ngramOperation(words, i, ngram_counts) {
...
function generalComputeBLEU(reflength, ngramMatches, numTstNgrams, smoothed, order) {
    ...
    let cumulative = prec.map(function (x) { return x * brevity; });
...
function bleuScoreObject(order, smoothed) {
    ...
    obj.update = function (otherBleuObject) {
...
function scoreSegment(tstSegment, refSegment) {
    //var numTstNgrams = [undefined].concat(zeroArray(maxN, Array));
    let numRefNgrams = [undefined].concat(zeroArray(maxN));
    let clippedRefNgrams = {};
    let reflengths = [];
...
exports.scoreSegment = scoreSegment;
