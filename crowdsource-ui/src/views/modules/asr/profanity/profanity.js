...
function getNextSentence() {
  ...
  if (currentIndex < window.asrInitiative.sentences.length - 1) {
    ...
    const encodedUrl = encodeURIComponent(window.asrInitiative.sentences[currentIndex].media);
...
function invokeProfanityStateUpdate(state, $skipButton, $submitButton, cancelButton) {
  ...
  updateProfanityState(localSpeakerDataParsed.userName, window.asrInitiative.sentences[currentIndex].dataset_row_id, localSpeakerDataParsed.language, state)
    .then(() => {
...
function updateSkipAction() {
  const sentenceId = window.asrInitiative.sentences[currentIndex].dataset_row_id;
...
function executeOnLoad() {
  ...
  try {
    ...
    if (isExistingUser && localSentencesParsed.sentences.length != 0 && localSentencesParsed.language === contributionLanguage) {
      ...
      setFooterPosition();
      window.asrInitiative.sentences = localSentencesParsed.sentences;
      initialize();
    } else {
      ...
      fetch(`/sentences-for-profanity-check/asr?username=${localSpeakerDataParsed.userName}&language=${contributionLanguage}`, {
        ...
        .then((sentenceData) => {
          ...
          $pageContent.removeClass('d-none');
          window.asrInitiative.sentences = sentenceData.data;
